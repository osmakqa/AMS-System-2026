...
const PrescriptionTable: React.FC<PrescriptionTableProps> = ({ items, onAction, onView, onUpdateWard, onUpdateTentativeDate, statusType, isHistoryView = false }) => {
  ...
  const getStatusColor = (status: string) => {
      ...
      if (s === 'deleted') return 'text-gray-500 bg-gray-200 line-through opacity-70';
        ...
        <table className="min-w-full divide-y divide-gray-200 border-collapse relative">
          <thead className="bg-green-50 sticky top-0 z-20 shadow-sm">
            <tr>
              ...
              {isHistoryView && (
                <th className="px-6 py-3 text-left text-xs font-medium text-green-800 uppercase tracking-wider">Tentative Last Dose</th>
              )}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {sortedItems.map((item, index) => {
              ...
              const isDeleted = item.status === PrescriptionStatus.DELETED;
              ...
              return (
                ...
                >
                  ...
                  {isHistoryView && (
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDeleted ? 'text-gray-400' : 'text-gray-500'}`}>
