    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[100] p-4 animate-fade-in backdrop-blur-sm" onClick={onClose}>
      <div className={`bg-white rounded-xl shadow-2xl w-full h-[90vh] flex flex-col border border-gray-200 overflow-hidden ${isReviewer ? 'max-w-[95vw]' : 'max-w-4xl'}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-green-700 text-white px-6 py-4 flex justify-between items-start shrink-0">
          <div>
            <div className="flex items-center gap-3">
                <h2 className="text-2xl font-bold">{item.patient_name}</h2>
                <span className="px-3 py-0.5 rounded-full text-[10px] font-black bg-white/20 border border-white/30 backdrop-blur-md uppercase tracking-widest">#{item.request_number}</span>
                <span className="px-3 py-0.5 rounded-full text-xs font-bold bg-white/20 border border-white/30 backdrop-blur-md">{formatStatus(item.status)}</span>
            </div>
            <div className="flex gap-4 mt-2 text-sm opacity-90">
                <p>Hosp ID: <span className="font-mono font-bold bg-green-800 px-2 py-0.5 rounded ml-1">{item.hospital_number}</span></p>
                <p>Date: <span className="font-bold ml-1">{item.req_date ? new Date(item.req_date).toLocaleDateString() : 'N/A'}</span></p>
            </div>
          </div>
          <button onClick={onClose} className="text-white/80 hover:text-white hover:bg-green-600 rounded-full p-2 transition-colors">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        {activeSection ? (
                            <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 shadow-sm animate-fade-in">
                                ...
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <select 
                                            className="w-full text-sm border-gray-300 rounded-md bg-white text-gray-900"
                            ...
                            <div className="text-center py-6 text-gray-400 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                <p className="text-sm">Select a section on the left to document clinical findings.</p>
                            </div>
                        )}

                        {findings.length > 0 && (
                            <div className="space-y-3 mt-4 pt-4 border-t border-gray-100">
                                <h5 className="text-xs font-bold text-gray-400 uppercase">Logged Interventions ({findings.length})</h5>
                                {findings.map((f, idx) => (
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-between items-center gap-4 shrink-0">
             <div className="text-[10px] font-mono text-gray-400">ID: {item.id}</div>
             <div className="flex gap-3">
                 {role === UserRole.PHARMACIST && item.status === PrescriptionStatus.PENDING && (
                    <>
                        {item.drug_type === DrugType.RESTRICTED ? (
                            <>
                                <button onClick={() => handleAction(ActionType.FORWARD_IDS)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-colors text-sm">For IDS Approval</button>
                                <button onClick={() => handleAction(ActionType.DISAPPROVE)} className="bg-white hover:bg-red-50 text-red-600 border border-red-200 px-4 py-2 rounded-lg font-bold shadow-sm transition-colors text-sm">Disapprove (Findings)</button>
                            </>
                        ) : (
                            <>
                                <button onClick={() => handleAction(ActionType.APPROVE)} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-colors text-sm">Approve</button>
                                <button onClick={() => handleAction(ActionType.DISAPPROVE)} className="bg-white hover:bg-red-50 text-red-600 border border-red-200 px-4 py-2 rounded-lg font-bold shadow-sm transition-colors text-sm">Disapprove (Findings)</button>
                            </>
                        )}
                    </>
                 ...
                 {role === UserRole.IDS && (item.status === PrescriptionStatus.FOR_IDS_APPROVAL) && (
                    <>
                        <button onClick={() => handleAction(ActionType.APPROVE)} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-colors text-sm">Approve</button>
                        <button onClick={() => handleAction(ActionType.DISAPPROVE)} className="bg-white hover:bg-red-50 text-red-600 border border-red-200 px-4 py-2 rounded-lg font-bold shadow-sm transition-colors text-sm">Disapprove</button>
                    </>
                 )}
                 {isReviewer && (
                    <button onClick={() => handleAction(ActionType.SAVE_FINDINGS)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-colors text-sm">Update Findings</button>
                 )}
                 <button onClick={onClose} className="px-5 py-2 bg-white hover:bg-gray-100 text-gray-700 rounded-lg font-bold transition-colors border border-gray-300 shadow-sm text-sm">Close</button>
             </div>
        </div>
      </div>
    </div>
